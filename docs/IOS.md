...
The iPhone app offers Sign in with Apple after the server advertises it in `/api/config`. Its native authorization code goes to the server for verification; the Apple private key stays on the server. An invited account can choose Apple before ever setting a password. When Apple shares the invited email, the first login connects automatically; when the person hides their email or uses a different Apple address, the one-time account link connects it without asking for a password. Unknown Apple accounts remain blocked while sign-up is closed. An Apple-only account can add a password in Settings after confirming with Apple again. An existing email/password account can link an Apple Account from Settings after confirming its password. The website and Mac app offer the same two login methods; the Mac completes Apple authorization in the browser and receives a one-time, device-bound handoff. The Mac opens the website's Account page to add a password to an Apple-only account.
