...
function buildTranscript(dir, base) {
  ...
  const spokenChars = cues.reduce((n, c) => n + c.text.replace(/\s+/g, '').length, 0);
...
class SummaryQueue extends EventEmitter {
  ...
  async _run(base) {
    ...
    if (countChars(body) > budget.cap) {
      ...
      const second = await api.stream({
        model: this.model,
        max_tokens: 16000,
        ...this.requestParams(),
        system: `你是一位严谨的编辑。用${LANG_NAMES[this.language] || this.language}输出 Markdown。`,
        messages: [{ role: 'user', content: `把下面这份学习摘要压缩到 ${budget.cap} 字以内（不含时间戳和 Markdown 符号），目标约 ${budget.target} 字。保持原有标题结构和每条要点末尾的时间戳；合并相近的要点，删掉例子、解释和重复，保留全部独立的结论。只输出压缩后的 Markdown。\n\n${body}` }],
      }).finalMessage();
