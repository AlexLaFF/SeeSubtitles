...
(function () {
  ...
  function renderList(root) {
    ...
    root.innerHTML = `
      ...
      <div class="toolbar"><div class="pills" id="filters"></div><div id="bulk" class="btns" style="margin:0" hidden></div><div class="grow"></div><input type="text" id="search" placeholder="${esc(t('files.search'))}" style="width:220px"></div>
      <div class="body" style="flex-direction:column;overflow:auto">
        <div class="ui" style="padding:0;overflow:hidden"><table><thead><tr><th style="width:28px"><input type="checkbox" id="selAll" title="${esc(t('files.selectAll'))}"></th><th style="width:32%">${t('files.col.name')}</th><th>${t('files.col.date')}</th><th>${t('files.col.length')}</th><th>${t('files.col.subtitles')}</th><th>${t('files.col.mp4')}</th><th>${t('files.col.summary')}</th><th></th></tr></thead><tbody id="rows"></tbody></table></div>
  ...
  async function renderDetail(root, base) {
    mounted = 'detail';
    root.innerHTML = `
      <div class="top"><h1><a class="crumb" id="btnBack" href="#">${t('files.crumb')}</a> <span id="dTitle"></span></h1><div class="chips"><span id="dChip" class="chip"></span></div><div class="grow"></div>
        <button id="btnResub" class="ghost">${t('files.resub')}</button><button id="btnMp4" class="ghost">${t('files.makeMp4')}</button><button id="btnSummary" class="ghost">${t('files.aiSummary')}</button><button id="btnDownload" class="primary">${t('files.download')}</button></div>
      <div class="body">
