...
build it rejects. `sh ios/scripts/testflight.sh` runs it, archives, uploads with the App Store Connect key in notarize.env and confirms. `npm run e2e:ios:real` runs the core against real keys on a throwaway copy of the server (about
...
## Multilingual live mode (0.8.6)

The `mixed` pipeline uses Alibaba `fun-asr-realtime` with automatic language detection and a 400 ms silence threshold, followed by the same rolling/final Hunyuan translation as the split pipeline (`server/lib/multilingual-stream.js`). Known-language live modes retain Tencent. The multilingual comparison and limits are in `docs/MULTILINGUAL-LIVE.md`. Do not reintroduce the earlier Gummy implementation from saved-work history: Fun-ASR superseded it after the paced comparison.
