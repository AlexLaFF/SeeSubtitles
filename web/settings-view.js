...
(function () {
  ...
  const hint = (t) => el('div', { class: 'hint', style: 'margin-left:138px' }, t);
  ...
  view.render = async function (root) {
    Controls.reset();
    mounted = true;
    root.innerHTML = `<div class="top"><h1>Settings</h1><span class="muted">Also in the menu bar: See Subtitles › Settings… (⌘,)</span></div>
    ...
    const sec = (id, title) => { const s = el('div', { class: 'ui', id: `sec-${id}` }); s.appendChild(el('h3', {}, title)); body.appendChild(s); return s; };
    ...
    const h = el('div', { class: 'hint', style: 'margin-left:138px' }); h.append(toggle, ' · Logging in gives this app its Tencent keys, the share link for remote displays and cloud re-subtitling.'); acc.appendChild(h);
    ...
    // ---- Tencent
    const tc = sec('tencent', 'Tencent Cloud (实时语音翻译)');
