...
The iPhone app offers Sign in with Apple after the server advertises it in `/api/config`. Its native authorization code goes to the server for verification; the Apple private key stays on the server. When `APPLE_SIGNUP_MODE=open`, a new Apple Account creates a See Subtitles account immediately, even if Apple supplies a private relay email. This is independent of password sign-up (`SIGNUP_MODE`); the latter may remain closed. A shared, verified email connects an existing account automatically. An existing account with a different Apple email or a hidden email uses its one-time setup link or links Apple from Settings so it is not mistaken for a new account. An Apple-only account can add a password in Settings after confirming with Apple again. The website and Mac app offer the same two login methods; the Mac completes Apple authorization in the browser and receives a one-time, device-bound handoff. The Mac opens the website's Account page to add a password to an Apple-only account.
